...
#### GraphQL
[GraphQL](https://graphql.org/) is an open source data query and manipulation language developed internally by Facebook. It provides an alternative to REST because it allows for a consistent way to make declarative queries. We first define the data structure(s) we need, then request the data and return exactly what was asked for. GraphQL enables small, efficient responses by only retrieving and returning exactly what was asked for, and nothing more. It also allows us to concatenate multiple calls into single data requests.

#### WPGraphQL
If you are choosing to use GraphQL on a Wordpress project, it is recommended to use the [WPGraphQL](https://github.com/wp-graphql/wp-graphql) plugin. This plugin will make all the data in the WordPress JSON API available through a GraphQL endpoint - in many cases you won’t need to write the schema yourself. This will give you all the benefits of concatenating your data requests as well as easy access to your data as it is output by WordPress. You can retrieve your data by passing a query directly into a simple fetch request:
